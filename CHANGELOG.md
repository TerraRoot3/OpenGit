...
- OpenGit 内置 Codex 会话新增跨会话工具：可查询本机正在执行的 Codex 会话、向指定会话追加或排队发送指令，并仅在用户明确要求时启动完成监控。
- Codex 会话监控会保存页面或飞书来源；任务结束后直接同步目标会话的最后一条回复，回复该通知时可继续向原 Codex 会话发送指令并再次监控。

### Changed

- 飞书群聊默认接收未 `@` 机器人的普通消息和附件，并继续按机器人配置与 `chat_id` 隔离 Codex 会话；消息存在明确提及时，仍只由被点名的机器人响应。
- 飞书监控通知复用现有机器人长连接，并以回复原监控请求的方式发送，不新增独立飞书桥接或常驻轮询；没有活动监控和排队消息时监控器会停止。
