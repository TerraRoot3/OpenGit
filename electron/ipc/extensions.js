...
function registerExtensionHandlers({ ipcMain, dialog, app, store, getMainWindow, safeLog, safeError }) {
  ...
  ipcMain.handle('get-extensions', async () => {
    try {
      ...
      safeLog(`🧩 获取扩展列表: ${extensionList.length} 个`)
      return { success: true, extensions: extensionList }
    } catch (error) {
      safeError('❌ 获取扩展列表失败:', error)
      return { success: false, message: error.message, extensions: [] }
    }
  })

  ipcMain.handle('load-extension-from-folder', async () => {
    try {
      ...
      const webviewSession = getWebviewSession()
      const extension = await webviewSession.loadExtension(extensionPath, { allowFileAccess: true })
      loadedExtensions.set(extension.id, { path: extensionPath, enabled: true })
      ...
      safeLog(`🧩 扩展加载成功: ${extension.name} (${extension.id})`)
      return { success: true, extension: { id: extension.id, name: extension.name, version: extension.version } }
    } catch (error) {
      safeError('❌ 加载扩展失败:', error)
      return { success: false, message: error.message }
    }
  })

  ipcMain.handle('load-extension-from-crx', async () => {
    try {
      const result = await dialog.showOpenDialog(getMainWindow(), {
        title: '选择扩展文件',
      ...
      return { success: false, message: '暂不支持直接加载 CRX 文件，请先解压扩展后从文件夹加载' }
    } catch (error) {
      safeError('❌ 加载 CRX 失败:', error)
      return { success: false, message: error.message }
    }
  })

  ipcMain.handle('load-extension-from-chrome', async () => {
    try {
      const webviewSession = getWebviewSession()
      ...
      for (const extId of extDirs) {
        ...
        try {
          ...
          if (existingExts.some(e => e.id === extId)) {
            safeLog(`🧩 扩展已加载，跳过: ${manifest.name || extId}`)
            continue
          }

          const extension = await webviewSession.loadExtension(extFullPath, { allowFileAccess: true })
          loadedExtensions.set(extension.id, { path: extFullPath, enabled: true })
          loadedExts.push({ id: extension.id, name: extension.name, version: extension.version })
          safeLog(`🧩 从 Chrome 加载扩展: ${extension.name}`)
        } catch (err) {
      ...
      safeError('❌ 从 Chrome 加载扩展失败:', error)
      return { success: false, message: error.message }
    }
  })

  ipcMain.handle('load-extension-by-id', async (event, extensionId) => {
    try {
      const webviewSession = getWebviewSession()
      const chromeExtPath = getChromeExtensionsPath()
      ...
      if (!fs.existsSync(extPath)) {
        return { success: false, message: `未找到扩展 ${extensionId}，请确保已在 Chrome 中安装该扩展` }
      }
      ...
      const extFullPath = path.join(extPath, versions[0])
      const existingExts = webviewSession.getAllExtensions()
      const existing = existingExts.find(e => e.id === extensionId)
      if (existing) {
        return { success: true, extension: { id: existing.id, name: existing.name, version: existing.version }, message: '扩展已加载' }
      }
      ...
      safeLog(`🧩 通过 ID 加载扩展: ${extension.name} (${extension.id})`)
      return { success: true, extension: { id: extension.id, name: extension.name, version: extension.version } }
    } catch (error) {
      safeError('❌ 通过 ID 加载扩展失败:', error)
      return { success: false, message: error.message }
    }
  })
    ...
    try {
      ...
      if (enabled) {
        ...
        if (extInfo && extInfo.path) {
          await webviewSession.loadExtension(extInfo.path, { allowFileAccess: true })
          loadedExtensions.set(extensionId, { ...extInfo, enabled: true })
        }
      } else {
        await webviewSession.removeExtension(extensionId)
        const extInfo = loadedExtensions.get(extensionId)
        if (extInfo) loadedExtensions.set(extensionId, { ...extInfo, enabled: false })
      }

      const savedExtensions = store.get('loadedExtensions', [])
      const updatedExtensions = savedExtensions.map(e => {
        const ext = loadedExtensions.get(extensionId)
        if (ext && e.path === ext.path) return { ...e, enabled }
        return e
      })
      store.set('loadedExtensions', updatedExtensions)
      safeLog(`🧩 扩展 ${extensionId} 已${enabled ? '启用' : '禁用'}`)
      return { success: true }
    } catch (error) {
      safeError('❌ 切换扩展状态失败:', error)
      return { success: false, message: error.message }
    }
  })
  ...
  ipcMain.handle('remove-extension', async (event, extensionId) => {
    try {
      ...
      safeLog(`🧩 扩展 ${extensionId} 已卸载`)
      return { success: true }
    } catch (error) {
      safeError('❌ 卸载扩展失败:', error)
      return { success: false, message: error.message }
    }
  })
