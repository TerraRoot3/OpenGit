...
function registerBrowserDataHandlers({ ipcMain, getMainWindow, store, safeLog, safeError }) {
  // 浏览器收藏功能
  ipcMain.handle('get-browser-favorites', async () => {
    try {
      const favorites = store.get('browserFavorites', [])
      return { success: true, favorites }
    } catch (error) {
      safeError('❌ 获取浏览器收藏失败:', error.message)
  ...
  ipcMain.handle('add-browser-favorite', async (event, { title, url, icon, domain }) => {
    try {
      ...
      safeError('❌ 添加浏览器收藏失败:', error.message)
      return { success: false, message: `添加失败: ${error.message}` }
    }
  })

  ipcMain.handle('remove-browser-favorite', async (event, { id }) => {
    try {
      if (!id) {
  ...
  ipcMain.handle('update-browser-favorite', async (event, { id, title, customColor, icon, sortOrder }) => {
    try {
      if (!id) {
        return { success: false, message: 'ID 不能为空' }
      }
  ...
  ipcMain.handle('save-browser-favorites-order', async (event, orderedIds) => {
    try {
  ...
  ipcMain.handle('export-browser-favorites', async () => {
    try {
      const favorites = store.get('browserFavorites', [])
  ...
  ipcMain.handle('import-browser-favorites', async () => {
    try {
      ...
      return { success: true, addedCount, updatedCount, totalCount: existingFavorites.length }
    } catch (error) {
  ...
  // 浏览器密码管理功能
  ipcMain.handle('get-browser-passwords', async () => {
    try {
      ...
      safeError('❌ 获取浏览器密码失败:', error.message)
      return { success: false, message: `获取失败: ${error.message}`, passwords: [] }
    }
  })

  ipcMain.handle('save-browser-password', async (event, { username, password, domain }) => {
    try {
      ...
      const passwords = store.get('browserPasswords', [])
      const existingIndex = passwords.findIndex(pwd => pwd.domain === domain && pwd.username === username)
      const passwordData = {
      ...
      if (existingIndex !== -1) {
        ...
        safeLog(`✅ 更新浏览器密码: ${domain} - ${username}`)
      } else {
        passwordData.createdAt = new Date().toISOString()
        passwords.push(passwordData)
        safeLog(`✅ 新增浏览器密码: ${domain} - ${username}`)
      }
      ...
      safeError('❌ 保存浏览器密码失败:', error.message)
      return { success: false, message: `保存失败: ${error.message}` }
    }
  })

  ipcMain.handle('get-browser-password', async (event, { domain, username }) => {
    try {
      const passwords = store.get('browserPasswords', [])
      const password = passwords.find(pwd => {
        const domainMatch = pwd.domain === domain || domain.includes(pwd.domain) || pwd.domain.includes(domain)
        if (username) return domainMatch && pwd.username === username
        return domainMatch
      })

      if (password) {
        return { success: true, username: password.username, password: password.password }
      }
  ...
  ipcMain.handle('update-browser-password-used', async (event, { id }) => {
    try {
      if (!id) {
        return { success: false, message: 'ID 不能为空' }
      }
      ...
      safeError('❌ 更新密码使用时间失败:', error.message)
      return { success: false, message: `更新失败: ${error.message}` }
    }
  })

  ipcMain.handle('clear-browser-passwords', async () => {
    try {
      store.set('browserPasswords', [])
      safeLog('✅ 已清除所有浏览器密码')
      return { success: true }
    } catch (error) {
      safeError('❌ 清除浏览器密码失败:', error.message)
      return { success: false, message: `清除失败: ${error.message}` }
    }
  })

  ipcMain.handle('delete-browser-password', async (event, { id }) => {
    try {
      if (!id) {
        return { success: false, message: 'ID 不能为空' }
      }
      ...
      safeError('❌ 删除浏览器密码失败:', error.message)
      return { success: false, message: `删除失败: ${error.message}` }
    }
  })

  ipcMain.handle('delete-browser-password-by-domain', async (event, { domain }) => {
    try {
      if (!domain) {
      ...
      safeLog(`✅ 删除域名 ${domain} 的密码: ${deletedCount} 个`)
      return { success: true, deletedCount }
    } catch (error) {
      safeError('❌ 删除域名密码失败:', error.message)
      return { success: false, message: `删除失败: ${error.message}` }
    }
  })
}

module.exports = { registerBrowserDataHandlers }
