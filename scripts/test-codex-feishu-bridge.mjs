      ...
      v1: {
        message: {
          ...
          reply: async (payload) => {
            ...
            return {
              code: 0,
              data: { message_id: 'om_monitor_notification' }
            }
          },
          get: async (payload) => {
...
assert.deepEqual(
  await bridge.sendText('oc_group', '监控完成', {
    replyToMessageId: 'om_monitor_request'
  }),
  {
    messageId: 'om_monitor_notification',
    messageIds: ['om_monitor_notification']
  }
)
assert.deepEqual(sentReplies, [{
  messageId: 'om_monitor_request',
  text: '监控完成'
}])
await bridge.stop()
