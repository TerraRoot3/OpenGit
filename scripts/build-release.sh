...
CSC_IDENTITY_AUTO_DISCOVERY=false bash scripts/run-electron-builder.sh --mac --arm64 --dir

# 步骤3：ad-hoc 签名
echo ""
echo "🔧 步骤3: 对应用进行 ad-hoc 签名..."
APP_PATH="$DIST_DIR/mac-arm64/OpenGit.app"
if [ -d "$APP_PATH" ]; then
  echo "📝 移除隔离属性..."
  xattr -cr "$APP_PATH" 2>/dev/null || true
  
  echo "📝 正在签名: OpenGit.app (ARM64)"
